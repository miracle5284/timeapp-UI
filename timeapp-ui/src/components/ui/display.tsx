...
const Display: React.FC<{value: string[] | number[], label: string, topComponent?: any[], downComponent?: any[]}> = (
    {value, label, topComponent, downComponent}) => {
    return (
        <div className="flex flex-col items-center text-white">
            <div className="flex">
                {value.map((char, index) => (
                    <>
                        <div className="flex flex-col justify-center items-center gap-2 opacity-80 pb-6">
                            {topComponent && topComponent[index]}
                            ...
                            {downComponent && downComponent[index]}
                        </div>
                    </>
                    ))}
            </div>
